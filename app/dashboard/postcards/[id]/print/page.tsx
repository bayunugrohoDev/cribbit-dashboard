...
export default async function PrintPostcardPage({
  ...
  if (error || !order) {
    return <div className="p-8 text-center text-red-500">Postcard not found</div>;
  }
    ...
    <div className="p-8 max-w-4xl mx-auto flex flex-col items-center">
      <div className="mb-8 print:hidden text-center flex flex-col items-center gap-4">
        ...
        <p className="text-muted-foreground text-sm max-w-md">
          Make sure your printer settings are set to A6 landscape (148 x 105 mm) with no margins.
        </p>
        <PrintButton />
      </div>
      ...
      >
        ...
        <div className="w-1/2 p-6 border-r border-slate-200 flex flex-col justify-between bg-slate-50">
          <div>
            ...
            <p className="text-[13px] text-slate-700 leading-relaxed mb-4">
              Jag är väldigt intresserad av din bostad på <strong>{addressLine1}</strong> och vill gärna komma i kontakt med dig.
            </p>
            <p className="text-[13px] text-slate-700 leading-relaxed">
              Skanna QR-koden till höger med din mobilkamera för att se mitt bud eller kontakta mig direkt, helt diskret.
            </p>
          </div>
          <div className="mt-4">
            <p className="text-[12px] font-semibold text-slate-800">Med vänlig hälsning,</p>
        ...
        <div className="w-1/2 p-6 flex flex-col relative bg-white">
          {/* Stamp placeholder */}
          <div className="absolute top-6 right-6 w-10 h-12 border border-slate-300 flex items-center justify-center text-[8px] text-slate-400">
            Frimärke
          </div>

          {/* QR Code */}
          <div className="mt-2 ml-4 flex flex-col items-start">
            <div className="p-1.5 bg-white rounded-lg border border-slate-100 shadow-sm inline-block">
              <QRCodeSVG value={`https://app.cribbit.se/p/${qrToken}`} size={70} />
            </div>
            <span className="text-[9px] mt-1 text-slate-500 font-medium tracking-wide">
              app.cribbit.se
            </span>
          </div>

          {/* Address */}
          <div className="mt-auto mb-2 border-l-[3px] border-black pl-3 ml-4">
            <p className="text-[11px] font-medium text-slate-500 mb-1">Mottagare:</p>
            <p className="text-[14px] font-bold text-slate-900 leading-tight">Till boende på</p>
            <p className="text-[14px] font-semibold text-slate-800 leading-tight">{addressLine1}</p>
            <p className="text-[14px] font-semibold text-slate-800 leading-tight uppercase">{postalTown}</p>
          </div>
        </div>
      </div>
    </div>
